...
silent). The agent core in `packages/agents` is **reused unchanged** except for the per-step
model-routing seam (1A.3); these tasks build around it.

---

## Phase 1A — RateEngine port (behaviour-preserving) + per-step model routing (D-07)
...
| # | Task | Proving test | Stop-after deliverable |
|---|---|---|---|
| **1C.1** | Trigger.dev **scheduled poll**: cursor + dedup by `graph_message_id` + re-enqueue stranded `received` requests (AUTONOMY W5). | **AC-1** (the same message id polled twice → exactly one `quote_request`) **and P-TENANT** (the poll for tenant A inserts only A's rows **and** re-enqueues only A's stranded `received` rows, never B's — `service_role` bypasses RLS, so this is the control). | Durable, tenant-scoped ingest. |
...
| **1C.4** | Approve → **simulated send**: status `sent`, `simulated_sent_at`, "SIMULATED SEND" badge, zero Graph send. | **AC-6** (sent only via Approve) **and AC-7** (zero send calls in the approve path) **and P-APPROVE-AUTH** (a reviewer in tenant A cannot approve/transition a tenant B request; B's `status` + `simulated_sent_at` stay unchanged). | The HITL commit, send-free, tenant-authorised. |
...
- **P-TENANT** — `service_role` tenant isolation across the **whole autonomous path** (RLS is bypassed
  there, so this — not AC-5 — is the control; AC-5 proves only the *browser* path). For a worker
  configured for tenant A: (i) every insert carries A's `tenant_id`; (ii) every read / re-enqueue
  (incl. stranded `received`) returns only A's rows; (iii) pricing reads `rate_card_lines` **only by
  joining through A's own `rate_cards.id`** — the child table has no `tenant_id`, so it is scoped by
  parent join, not a column filter. Seed **two** tenants with **distinct rate lines + stranded rows**;
  one tenant cannot catch a leak.
- **P-APPROVE-AUTH** — cross-tenant approve rejected: a reviewer authenticated to tenant A cannot
  approve/transition a tenant B request; B's `status` + `simulated_sent_at` stay unchanged.
...
- **1A before everything:** the port is a pure refactor; proving it is behaviour-equivalent
  ...
  unchanged, already-verified core. Model routing (1A.3) lands here too — it is the one
  model-affecting change, so it re-runs the golden set before any production plumbing depends on it.
...
## Definition of done
- **1A:** port defined, pricing behind it, refactor-equivalent (**P-1A.2**), per-step model routing
  (**P-1A.3**), **live eval ≥6/8** unchanged from Phase 0.
- **1B:** schema + RLS isolation (AC-5), card as rows (P-1B.2), Supabase adapter at parity and
  golden-set ≥6/8 (AC-3 + AC-2), snapshots reproducible (AC-4), Graph wrapper send-free at scope +
  method + call level + read-proven (AC-7 + P-1B.5); Excel either swapped in at parity and write-free
  (AC-3 + P-EXCEL-RO) **or** a logged fallback to Supabase.
- **1C:** ingest dedup (AC-1), idempotent run (P-1C.2), **`service_role` tenant isolation across
  inserts/reads/child-rows (P-TENANT)**, tenant-isolated **and correctly-rendered** dashboard
  (AC-5 + P-1C.3), approve-gated simulated send with **cross-tenant approve rejected**
  (AC-6 + AC-7 + P-APPROVE-AUTH), escalation/injection surfaced (AC-8), usage observable (P-1C.6).
