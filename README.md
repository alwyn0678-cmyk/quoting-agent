...
[![CI](https://github.com/alwyn0678-cmyk/quoting-agent/actions/workflows/ci.yml/badge.svg)](https://github.com/alwyn0678-cmyk/quoting-agent/actions/workflows/ci.yml)
![tests](https://img.shields.io/badge/tests-215%20passing-2ea44f?style=flat-square)
![vulnerabilities](https://img.shields.io/badge/npm%20audit-0%20vulnerabilities-2ea44f?style=flat-square)
...
```bash
npm test          # 215 offline tests — no keys, no network
...
- **RAG is scoped to the reply prose, never the price** ([Q3](docs/AGENT_DESIGN.md)). Retrieval grounds
  *explanations* of charges/terms (Gemini embeddings + Supabase pgvector) and runs **after** pricing, so
  a retrieved chunk is structurally incapable of moving a number — and a retrieval *failure* degrades to
  an ungrounded draft instead of crashing the run. Knowing when **not** to use RAG is the point.
- **A hard trusted/untrusted boundary — at both LLM steps.** The email is untrusted: extraction reads it
  inside an escaped `<email>` data block, turning free text into a constrained schema. The draft step
  gets engine-verified figures, and the few email-derived text fields it needs (names, lane text,
  commodity) are escaped and confined to a `<customer_fields>` data block of their own.
- **Fail closed, three guard layers.** The gate guarantees a `quote` is priceable; an injection guard
  re-derives the price **against the same card the gate validated** and checks for prompt-leak /
  price-tampering / draft-total drift; a final canary net scans every output. Any violation → drop the
  quote and **escalate to a human**.
...
- **Domain humility.** Every rate, surcharge, and incoterm rule is **invented** and logged in
  ...
  stated as fact.
- **Audited, adversarially.** Every phase ships through a self-review + independent second-opinion
  gate, reconciled in [`docs/AUDIT_LOG.md`](docs/AUDIT_LOG.md) — including a full-repo hardening sweep
  (five parallel auditors) whose one critical finding came with a composed-pipeline regression test.
  Known gaps are logged there, not hidden.
...
CI runs the hermetic suite + all typecheck roots on every push.
...
account/credential setup. A **2026-07 full-repo audit + hardening sweep** (multi-modal guard fix,
mail-loss pagination, dashboard resilience, SQL hardening migration 0015, 0 npm vulnerabilities) is
documented in [AUDIT_LOG.md](docs/AUDIT_LOG.md). See the docs above for what's live vs. deferred.
